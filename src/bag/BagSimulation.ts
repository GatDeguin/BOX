...
export interface BagSimulationOptions {
    world: CANNON.World;
    scene: THREE.Scene;
    noise: { noise2D(x: number, y: number): number };
    visualConfig: any;
    punchConfig: any;
    collisionGroups: { static: number; chain: number; bag: number; fist: number };
    geometryDetail: { radialSegments: number; heightSegments: number };
    bagVisualOffset?: THREE.Vector3;
    mobileSimpleMode?: boolean;
}

export interface BagMaterials {
    bagMaterial: CANNON.Material;
    chainMaterial: CANNON.Material;
    ceilingMaterial: CANNON.Material;
    staticMat: CANNON.Material;
}

export interface BagTextures {
    c: THREE.Texture;
    n: THREE.Texture;
    r: THREE.Texture;
}
...
export class BagSimulation {
    ...
    private noise: { noise2D(x: number, y: number): number };
    private visualConfig: any;
    private punchConfig: any;
    private collisionGroups: BagSimulationOptions['collisionGroups'];
    private geometryDetail: BagSimulationOptions['geometryDetail'];
    private mobileSimpleMode: boolean;

    private bagBody: CANNON.Body | null = null;
    private bagMesh: THREE.Mesh | null = null;
    private chains: { m: THREE.Mesh; b: CANNON.Body }[] = [];
    private bagVisualOffset: THREE.Vector3;
    private lastVisualDt = 1 / 60;

    private originalPositions: THREE.Vector3[] = [];
    private vertexVelocities: THREE.Vector3[] = [];

    private leftPunchCooldown = 0;
    private rightPunchCooldown = 0;

    private basePosition: CANNON.Vec3 | null = null;
    private baseQuaternion: CANNON.Quaternion | null = null;

    constructor(options: BagSimulationOptions) {
    ...
    initialize(textures: BagTextures, materials: BagMaterials) {
        this.resetInternalState();
        this.createBag(textures, materials);
        this.captureBaseState();
    }

    applyPunch(handVelocityVec3: { x: number; y: number; z: number }, contactPointVec3: { x: number; y: number; z: number }, isLeft: boolean): PunchResult | null {
        if (!this.bagBody || !this.bagMesh) return null;
        ...
        const normalizedImpact = THREE.MathUtils.clamp(
            punchData.impulseMagnitude / this.visualConfig.bag.punchForce,
            0.15,
            1.5
        );
        this.deform(
            new THREE.Vector3(punchData.contactPoint.x, punchData.contactPoint.y, punchData.contactPoint.z),
            normalDir,
            this.lastVisualDt,
            normalizedImpact
        );

        if (isLeft) this.leftPunchCooldown = 0.25; else this.rightPunchCooldown = 0.25;
    ...
    private getBagPointVelocity(contactPoint: CANNON.Vec3) {
    ...
    private computePunchImpulse(handVelocityVec3: { x: number; y: number; z: number }, contactPointVec3: { x: number; y: number; z: number }, isLeft: boolean) {
        if (!this.bagBody) return null;
    ...
    private deform(point: THREE.Vector3, dir: THREE.Vector3, dt?: number, intensity = 1) {
        if (!this.bagMesh) return;
    ...
    private createBag(tex: BagTextures, materials: BagMaterials) {
        ...
        let geo = new THREE.CylinderGeometry(
            ...
            r,
            h,
            this.geometryDetail.radialSegments,
            this.geometryDetail.heightSegments
        );
